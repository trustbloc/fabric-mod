...
func checkDataNotExists(t *testing.T, s xstorageapi.PrivateDataStore, blkNum int) {
	data, err := s.GetPvtDataByBlockNum(uint64(blkNum), nil)
	require.NoError(t, err)
	require.Nil(t, data)
}

func checkDataExists(t *testing.T, s xstorageapi.PrivateDataStore, blkNum int) {
	data, err := s.GetPvtDataByBlockNum(uint64(blkNum), nil)
	require.NoError(t, err)
