...
run_tests() {
    ...
    time {
        ...
        if [ "${#parallel[@]}" -ne 0 ]; then
            go test "${flags[@]}" "${race_flags[@]}" -tags "$GO_TAGS" "${parallel[@]}" -short -timeout=90m -failfast ${PARALLEL_NUM}
        fi
    }
}
...
run_tests_with_coverage() {
    ...
    time go test -p 1 -cover -coverprofile=profile_tmp.cov -tags "$GO_TAGS" "$@" -timeout=90m
