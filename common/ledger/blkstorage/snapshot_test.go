...
func TestBootstrapFromSnapshot(t *testing.T) {
	...
	var snapshotInfo *extledgerapi.SnapshotInfo
	var bootstrappedBlockStore extledgerapi.BlockStore
	...
	setup := func() {
		...
		snapshotInfo = &extledgerapi.SnapshotInfo{
	...
	t.Run("sync-up-indexes", func(t *testing.T) {
		...
		for i, blockDetail := range blocksDetailsAfterSnapshot {
			...
			blkfileMgr := bootstrappedBlockStore.(*BlockStore).fileMgr
			originalIndexDB := blkfileMgr.index.db
			bootstrappedBlockStore.(*BlockStore).fileMgr.index.db = env.provider.leveldbProvider.GetDBHandle(filepath.Join(testDir, "someRandomPlace"))
...
func verifyQueriesOnBlocksPriorToSnapshot(
	t *testing.T,
	bootstrappedBlockStore extledgerapi.BlockStore,
	expectedBCInfo *common.BlockchainInfo,
	blocksDetailsBeforeSnapshot []*testBlockDetails,
	blocksBeforeSnapshot []*common.Block,
) {
