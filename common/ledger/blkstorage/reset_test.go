...
func TestResetToGenesisBlkMultipleBlkFiles(t *testing.T) {
	...
	for i, b := range blocks {
		...
		if i != 0 && i%blocksPerFile == 0 {
			...
			store.(*BlockStore).fileMgr.moveToNextFile()
		}
	}
	store.Shutdown()
	provider.Close()
...
func assertBlockStorePostReset(t *testing.T, store extledgerapi.BlockStore, originallyCommittedBlocks []*common.Block) {
